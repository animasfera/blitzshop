...
export interface BoxberryCountry {
  Code: string
  NameRu: string
  NameEng: string
...
export const getBoxberryListCountries = async ({ lang }: { lang: LocaleEnum }) => {
  ...
  try {
    ...
    if (data.ok) {
      ...
      const countryList = res.result.map((el) => {
        ...
        return {
          value: el.Code,
          label: lang === LocaleEnum.ru ? el.NameRu : el.NameEng,
          img,
        }
      })
      ...
      const otherCountries = [
        {
          value: "051",
          label: lang === LocaleEnum.ru ? "Армения" : "Armenia",
          img: getUrlСountryFlag({ country: "AM" }) ?? "",
        },
        {
          value: "417",
          label: lang === LocaleEnum.ru ? "Киргизия" : "Kyrgyzstan",
          img: getUrlСountryFlag({ country: "KG" }) ?? "",
        },
        {
          value: "762",
          label: lang === LocaleEnum.ru ? "Таджикистан" : "Tajikistan",
...
export default resolver.pipe(
  ...
  async ({}, ctx: Ctx) => {
    ...
    const countries = await getBoxberryListCountries({
      lang: ctx.session.user?.locale ?? LocaleEnum.en,
    })
